...
/// 远程敌人眩晕控制器：负责眩晕状态管理
/// </summary>
public class RangeEnemyStunController : MonoBehaviour
{
    [Header("眩晕设置")]
    ...
    [Header("击退设置")]
    [SerializeField] private bool stunOnKnockback = false;
    ...
    private void Awake()
    {
        ...
        {
            animationController = GetComponent<RangeEnemyAnimationController>();
        }
        
        if (showStunDebug)
        {
            Debug.Log($"🌀 [RangeStun] Awake: {gameObject.name}");
        }
    }
    
    public void Initialize(RangeEnemyAnimationController animController = null)
    {
        ...
        if (showStunDebug)
        {
            Debug.Log($"✅ [RangeStun] 初始化完成");
            Debug.Log($"   - 击退时眩晕: {stunOnKnockback}");
        }
    }
    ...
    public void ForceEndStun()
    {
        ...
        if (showStunDebug)
        {
            Debug.Log($"🛑 强制结束眩晕: {gameObject.name}");
        }
    }
